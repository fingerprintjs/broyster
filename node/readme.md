...
Requires Node.js ≥ 20.19 (the Vite 8 floor). `vitest` and `@vitest/browser` are peer dependencies.
...
The parent process and the Vitest child processes communicate through a versioned JSON environment
contract (`BROYSTER_CHILD_CONTEXT`); you never have to set it yourself. The selected browser
definition is included in that contract, so the orchestrator's catalog is the single source of truth.
...
Broyster passes the tunnel token to `cloudflared` through its private `TUNNEL_TOKEN` environment
variable rather than the process argument list. Tunnel-owned token variables are removed before
Vitest children are spawned. Startup is confirmed through `cloudflared`'s local metrics readiness
endpoint and retried with bounded, force-kill-capable cleanup when necessary.
...
| `browsers` | — | Deprecated compatibility option; the orchestrator's catalog is authoritative |
...
| `vite` | — | Extra Vite config; safe settings are preserved and runner-owned fields are enforced |

### Browser catalog

The default catalog (`browserstackBrowsers` from `@fpjs-incubator/broyster`) covers Windows, macOS,
Android and iOS. To use your own targets, define one catalog module and pass it to the orchestrator.
The selected definition is serialized into each Vitest child, avoiding duplicated configuration:
...
export default createBrowserStackConfig({ projectName: 'MyProject', ... })
```

```ts
// run.ts — custom catalogs use the programmatic API
...
const config = defineBroysterConfig({
  ...
  // buildName, credentials, retryFailed, cwd, env, resultsFile, signal, debug, onLog
})
const summary = await runBrowserStackTests(config)
...
### Result artifacts and cancellation

Set `resultsFile` programmatically or pass `--results <path>` to write an atomic JSON artifact.
Artifacts have a `schemaVersion`, stable browser/module/test ordering, full structured attempt
results, and an effective module state. A failed-file retry overlays only the files it reran; a
whole-browser retry replaces the initial module state.

Pass an `AbortSignal` as `signal` to cancel a programmatic run. Broyster stops launching browsers,
interrupts queue/port waits, terminates active Vitest children with bounded `SIGTERM` → `SIGKILL`
cleanup, waits for all active work to settle, and then closes the transport.
