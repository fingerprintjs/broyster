...
describe('BrowserStackApiClient', () => {
  ...
  it('redacts credentials from bounded error response bodies', async () => {
    mockFetch({
      ok: false,
      status: 500,
      text: () => Promise.resolve(`username=user accessKey=key auth=${expectedAuthorization} ${'x'.repeat(3_000)}`),
    })

    const client = new BrowserStackApiClient(credentials)
    const error = await client.setSessionStatus('abc', 'failed').catch((caught: unknown) => caught)

    expect(error).toBeInstanceOf(Error)
    expect((error as Error).message).not.toContain('user')
    expect((error as Error).message).not.toContain('key')
    expect((error as Error).message).not.toContain(expectedAuthorization)
