...
export class BrowserStackProvider implements BrowserProvider {
  ...
  private resolveCapabilities(): BrowserStackCapabilities | undefined {
    const sharedLocalIdentifier =
      BrowserStackSessionManager.getTunnelIdentifier() ?? process.env.BS_LOCAL_IDENTIFIER ?? null
    const capabilities = this.options.capabilities

    // Enable BrowserStack Local when a shared identifier is available unless explicitly disabled.
    ...
    // When using public-url mode, local tunnel is off unless the consumer forces it.
    if (process.env.BS_PUBLIC_BASE_URL && capabilities?.local === undefined) {
      return {
        ...capabilities,
        local: false,
      }
  ...
  /**
   * Rewrites the Vitest page URL for remote browsers:
   * - BS_PUBLIC_BASE_URL: consumer-managed public tunnel (e.g. Cloudflare)
   * - BrowserStack Local: map localhost/127.0.0.1 → bs-local.com (required for Safari/iOS)
   */
  private resolvePageUrl(url: string): string {
    ...
    if (pageUrl.hostname === 'localhost' || pageUrl.hostname === '127.0.0.1' || pageUrl.hostname === '0.0.0.0') {
      // BrowserStack Local exposes the machine as bs-local.com to remote browsers.
      pageUrl.hostname = 'bs-local.com'
    }
    return pageUrl.toString()
  }
