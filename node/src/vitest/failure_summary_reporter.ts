...
export class FailureSummaryReporter implements Reporter {
  ...
  onTestCaseResult(testCase: TestCase): void {
    ...
    if (!isFailedTestCaseResult(result)) {
      return
    }
  ...
  onTestRunEnd(testModules: ReadonlyArray<TestModule>, unhandledErrors: ReadonlyArray<unknown>): void {
    ...
    const runLabel = process.env.BS_BROWSER || projectNames.join(', ')
...
function isFailedTestCaseResult(result: ReturnType<TestCase['result']>): boolean {
