...
import {
  ...
  normalizeTestStatus,
  serializeError,
} from '../../contracts/attempt_report.js'
import type { AttemptReport } from '../../contracts/attempt_report.js'
...
import type { TestCaseResult, TestModuleResult } from '../../orchestrator/results.js'

export type FailedFilesReport = AttemptReport
...
export class FailedFilesReporter implements Reporter {
  ...
  async onTestRunEnd(testModules: ReadonlyArray<TestModule>, unhandledErrors: ReadonlyArray<unknown>): Promise<void> {
    ...
    const report: FailedFilesReport = {
      ...
      modules: testModules.map(serializeModule).sort(compareById),
      unhandledErrors: unhandledErrors.map(serializeError),
      warnings: [],
    }

    await mkdir(dirname(outputPath), { recursive: true })
    await writeFile(outputPath, formatJson(report), 'utf8')
  }
}

function serializeModule(testModule: TestModule): TestModuleResult {
  return {
    id: testModule.moduleId,
    status: normalizeModuleStatus(testModule.state()),
    duration: testModule.diagnostic().duration,
    errors: testModule.errors().map(serializeError),
    tests: [...testModule.children.allTests()].map(serializeTest).sort(compareById),
  }
}

function serializeTest(testCase: TestCase): TestCaseResult {
  const result = testCase.result()
  return {
    id: testCase.id,
    name: testCase.name,
    fullName: testCase.fullName,
    status: normalizeTestStatus(result.state, testCase.options.mode === 'todo'),
